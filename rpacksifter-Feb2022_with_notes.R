...
#1 sifter main function: handles numeric, categorical/factor (up to 50 levels),
# unstructured (only one column), 
#  dates (detects, reformat as dates, shift them +-1/2days)
###########################################################################
dataSifter<-function(level="indep",data,unstructured.names=NULL,subjID=NULL,batchsubj=1000,
                     missingback=FALSE,usecore=2,col_preserve = 0.5, col_pct = 0.7,
  ...
  if(!is.null(subjID)) 
    {
    data<-data[,-which(names(data) %in%subjID)]
  }
  ...
  if(!is.null(sddata[[2]])) {
    ...
    data <- data[,-dateind]
    #print(names(data))
    #Date<-Sifter_Dates(Date)
    ## Below is obsolete, since I updated the function Sifter_Dates()
    ## to handle different sized Date
    if(length(dateind)>1){
      Date <- as.data.frame(lapply(Date,function(x) Sifter_Dates(x)))
      names(Date) = Datename} else {
        Date<-Sifter_Dates(Date)
        }
  }
  # Separate into batches and do imputation,
  # if there are less than 1000 we don't separate by defult.
  if(nomissing==FALSE){
    unind<-which(names(data) %in%  unstructured.names)
    if(nrow(data)>batchsubj){
      ...
      if(length(unind)!=0){data.imp<-cbind.data.frame(data[,unind],data.imp);
      names(data.imp)[1]=unstructured.names}
    }
  ...
  if(level=="none"&is.na(sum(k0+k1+k2+k3+k4))){
    ...
    } else if(length(listfac)==1)
    {
      factable<-list(table(data[,listfac])); names(factable)=names(data)[listfac]
      }
    ...
    if (length(list_string)==0) #if (is.null(list_string)) {
      {
...
firstImp<-function(sepdata,unstructured.list=NULL,firstimputation=FALSE,cores=2,maxiter=1){
  ...
  if(firstimputation==TRUE){
    ...
    if(!is.null(unstructured.list)){
      ...
      K <- 
        foreach::foreach(i=1:numgroup,.combine=rbind.data.frame,.packages = c("missForest")) %dopar% {missForest(sepdata[[i]][,-c(which(names(sepdata[[i]])=="ID"))],maxiter=maxiter)$ximp}
    }
...
sep<-function(data,numsample=1000){#change the name to batching
  ...
  #subsetting the original dataset according to case ID groups and assign names or save into a list
  sepdata<-list()
  for(i in 1:numgroup){
...
subjdist<-function(completedata,unstructured.list=NULL){
  ...
  if(!is.null(unstructured.list))
  {
    completedata[,unstructured.list]<-sapply(completedata[,unstructured.list],as.character)
    }
  numcols<-which(sapply(completedata,class) %in% c("numeric","integer"))
  ## Check if there is no numerical feature
  # if (length(numcols) < 1) {
  #   cat("NO NUMERICAL FEATURE IN THE DATASET !! ONLY GOWER DISTANCE IS CALCULATED")
  #   #data_temp_1$dummy <- runif(dim(data_temp_1)[1])
  # }
  
#  catcols<-which(sapply(completedata,class) %in% c("character","factor"))
  catcols<-which(sapply(completedata,class) %in% c("factor"))
  if(!is.null(unstructured.list)) 
  {
    catcols<-catcols[-which(catcols%in% unstructured.list)] 
    }
  ## If there is at least 1 numeric feature]
  if (length(numcols) > 1) {
    ...
    if(length(catcols)<=1) {#ignored when single catagory data is available
      ...
      subjDist_sorted <- subjDist[order(subjDist$Distance), ] } else {
        ...
        print("b has been generated")
        }
    subjDist = data.frame(t(b))
    subjDist[1:2,]
    names(subjDist) = c("Case j", "Case i", "catdist")
    subjDist$Distance <- 
      subjDist$catdist*length(catcols)/ncol(completedata) # + unstructured.dist
    subjDist<-subjDist[,-3]
    ...
    #numdist<-(numdist-min(numdist))/(max(numdist)-min(numdist))#scale to 0-1
    comb_subjects = combn(1:nrow(completedata), 2)
    if(length(catcols) <=1) {#ignored when single category data is available
      b = rbind(comb_subjects, numdist)
      subjDist = data.frame(t(b))
      names(subjDist) = c("Case j", "Case i", "Distance")
      subjDist_sorted <- subjDist[order(subjDist$Distance), ]
      ...
      catdist<-daisy(completedata[,catcols],metric="gower")
      catdist<-(catdist-min(catdist))/(max(catdist)-min(catdist))
      b = rbind(comb_subjects, catdist) #numdist
    }  
    subjDist = data.frame(t(b))
    names(subjDist) = c("Case j", "Case i", "catdist")
    subjDist$Distance <- 
      subjDist$catdist*length(catcols)/ncol(completedata) # + unstructured.dist
    ## order
    order_list <- order(subjDist$Distance)
    subjDist<-subjDist[,-3]
    subjDist_sorted <- subjDist[order_list, ]
  }
...
#   ## number of records
#   n=dim(date_col)[1]
#   ## number of date-like fields
#   k=dim(date_col)[2]
#   #date_col<-as.Date(date_col)
#   for(i in 1:k) {  
#     date_col_temp <- error <- date <- id <- NULL
#     date_col_temp <- data.frame(id=1:n,date_col[,i])
#     ## order the dates, starting from the oldest
#     date <- date_col_temp[order(date_col_temp[,2]),2]
#     id <- date_col_temp[order(date_col_temp[,2]),1]
#     #Calculate the distance among all the adjacent dates [(x)-(x-1)]
...
#       floor(rtruncnorm(1,a=-min(datedist[x],datedist[x+1]),
#                        b =min(datedist[x],datedist[x+1]),
#                        mean = 0,sd = 0.5*min(datedist[x],datedist[x+1]))))
#     ## Set to 0 any NaN in error
#     error[which(is.na(error)==T)] <- min(0,abs(error),na.rm = T)
#     # Follows a normal distribution with sd= 0.5*min(dist[i],dist[i+1])
#     date_col_temp <- 
#       data.frame(id=id,sifterdate=as.Date(date+error,origin="1970-01-01"))
#     date_col_temp <- date_col_temp[order(date_col_temp$id),2]
#     date_col[,i] <- date_col_temp
#   }
#   return(date_col)
# }
